...
\end{longtable}
\subsection{Cost Analysis Selected Pricing Options}
...
Considering the parameters and pricing options defined in Subsection \ref{Subsection:Cost_Analysis_Parameters}, the estimated monthly cost of hosting the web \ac{API} in each cloud platform was calculated. Table \ref{Table:Cost_Analysis_Results} indicates the results in Euros.
...
The results indicate that \ac{AWS} is the cheapest cloud platform to host the web \ac{API}, with an estimated monthly cost of 1,106.88 \euro. The most expensive option is Microsoft Azure, with 1,480.38 \euro. This higher cost is mainly due to the \ac{API} Gateway, which requires a more expensive pricing tier to support \ac{VNet} integration. In-between is \ac{GCP}, with an estimated monthly cost of 1,207.01 \euro.
