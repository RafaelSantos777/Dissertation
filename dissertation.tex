...
\begin{longtable}{|M{70pt}|M{55pt}|m{300pt}|}
  ...
  Monthly Data Egress
   & 500 GB
   & Considering value of the "Monthly API Requests" parameter, this amounts to an average of 5 \ac{KB} per request response. The size of the responses delivered to users varies heavily, ranging from less than 1 \ac{KB}, to more than 100 \ac{KB} for some quote history requests. 5 \ac{KB} represents a typical quote request response size.                                                                                                                          \\
  \hline
  ...
  Microservice CPU
   & 1 \ac{vCPU} (each)
   & This value represents the average \ac{CPU} utilization of all instances (replicas) of a microservice combined. In the implemented web \ac{API}, each instance is allocated 0.5 \acp{vCPU}. Therefore, the value of this parameter assumes that the average \ac{CPU} utilization is equivalent to 2 instances running at full capacity. All chosen \acp{vCPU} belong to a general-purpose tier, although their processing power may still vary between cloud providers. \\
  \hline
  Microservice Memory
   & 2 GB (each)
   & Similarly to the "Microservice CPU" parameter, this value refers to the average memory utilization combining all instances (replicas) of a microservice. In the web \ac{API}, each instance has 1 \ac{GB} of memory. Thus, it is assumed that the average memory use is identical to 2 instances running at full capacity.                                                                                                                                             \\
  \hline
  Number of Databases
   & 3
  ...
  Database CPU
   & 16 \acp{vCPU} (each)
   & It is difficult to predict the necessary \ac{CPU} resources for the databases due to varying query complexities and usage patterns. Quotation requests involve more resource-intensive queries than the ones used in the performance test, so more than 2 \acp{vCPU} were selected. The value of this parameter is also associated with mid-tier options in cloud provider offerings.                                                                                  \\
  \hline
  Database Storage
   & 128 GB (each)
