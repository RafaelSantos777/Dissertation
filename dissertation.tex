...
The \ac{VNet} provides network isolation for the system's components in the cloud platform. The Microservice Environment and the API Gateway reside in the \ac{VNet}, each in its dedicated subnet. The main objective of these dedicated subnets is to implement network segmentation, enforcing network policies that apply to all inbound and outbound traffic. Specifically, the \ac{API} Gateway subnet is configured to allow inbound traffic from the Internet, while the Microservice Environment subnet only accepts traffic from within the \ac{VNet}. Furthermore, components within the \ac{VNet} can securely access other components (the Message Broker, the Azure Key Vault, and the Data Persistence Layer) via private endpoints, which integrate them directly into the \ac{VNet}'s address space.
...
Different databases may have differing computation power and storage capacity, depending on the microservice's needs. Since the web \ac{API} is a proof-of-concept, all databases have 16 \acp{GB} of storage, to save costs. In terms of computation power, Quotation Service's database has 2 \acp{vCore}, while the other microservices' databases have the default 1 \ac{vCore}. This disparity is due to Quotation Service needing to store and lookup not only users' quotes, but also locations and special handling codes. Figure \ref{Figure:DatabaseConfigurationBase} shows the basic configuration properties of Quotation Service's database, which is similar to all others barring the number of \acp{vCore}.
...
\begin{longtable}{|c|m{280pt}|c|}
  ...
  % The caption and label are moved inside the longtable environment.
  \caption{Requirements Fulfillment Explanation}
  \label{Table:Requirements_Fulfillment}                                                                                                                                                                                                                                                       \\

  \hline
  % This header appears on the first page.
  ...
  % This header is repeated on all subsequent pages.
  % \caption[]{(Continued) Requirements Fulfillment Explanation}                                                                                                                                                                                                                                                \\
  \hline
  \multicolumn{1}{|c|}{\textbf{Requirement}}
   & \multicolumn{1}{c|}{\textbf{Fulfillment Explanation}}
   & \multicolumn{1}{c|}{\textbf{Subsections}}                                                                                                                                                                                                                                                 \\
  ...
  % This footer appears on all pages except the last one.
  \hline
